...
list_of_files = glob.glob(os.path.join(ROOT, 'data/CC3M/images/*'))
list_of_prompts = glob.glob(os.path.join(ROOT, 'data/CC3M/prompts/*'))
...
with open(os.path.join(ROOT, 'data/CC3M/val_data.json'), 'wt') as f:
    for i in range(len(list_of_files)):
        ...
        f.write(json.dumps({'target': list_of_files[i], 'source': list_of_files[i].replace('images', 'edges'), 'prompt': prompt, 'ds_label': 'CC3M'}) + '\n')


os.makedirs(os.path.join(ROOT, 'data/CC3M/edges/'), exist_ok=True)
